...
struct Code : public Xbyak::CodeGenerator {
	...
	// zm0 = log(zm0)
	// use zm0, zm1, zm2
	void genLogOneAVX512(const Args& t, const LogParam& p, const Label& constVarL)
	{
#ifdef LOG_TBL
//int3();
		vmovaps(t[4], t[0]);
		...
		kxnord(k2, k2, k2);
		lea(rax, ptr[rip+constVarL]);
//		mov(rax, (size_t)constVarL.getAddress());
...
//		mov(t[0], p1, p.fMInf);
		...
		vfmsub213ps(t[0], p.f1div3, p.logCoeff[0]); // a
...
printf("i127shl23=%s\n", para.i127shl23.toString());
printf("x7fffff=%s\n", para.x7fffff.toString());
printf("log2=%s\n", para.log2.toString());
printf("one=%s\n", para.one.toString());
printf("half=%s\n", para.half.toString());
printf("sqrt2=%s\n", para.sqrt2.toString());
...
printf("keepRegN=%d\n", keepRegN);
	...
	Label lp = L();
		...
		add(src, 64);
		genLogOneAVX512(args, para, constVarL);
